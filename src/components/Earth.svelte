<div>
    <video width="100%" height="50%" autoplay muted loop>
        <source src="earth.mp4" type="video/mp4">
        Your browser does not support the video tag.
    </video>
</div>

<div class="z-10 absolute mt-[-30%] left-1/2 transform -translate-x-1/2 -translate-y-1/2 text-center">
    <div class="rounded-xl p-8 mb-4 bg-[#092b5b] opacity-70">
        <div>
            <h3 class="fadeInAnimated font-bold mb-2 text-[#A284B6] text-6xl">ALLEGRA-Consult</h3>
            <p class="fadeInAnimated text-[#A284B6] text-4xl">A Global Consultancy</p>
        </div>
    </div>
</div>

<div class="z-10 absolute mt-[-50%] left-80 transform -translate-x-1/2 -translate-y-1/2 text-center">
    <div class="shadow-md mb-4 p-10 bg-[#00000] opacity-100 font-extrabold">
        <p class="text-justify hover:text-[#A284B6] fadeIn2s dark:text-slate-400 text-xl dark:text-white">Working alliances</p>
    </div>
</div>

<div class="z-10 absolute mt-[-40%] left-64 transform -translate-x-1/2 -translate-y-1/2 text-center">
    <div class="shadow-md mb-4 p-10 bg-[#00000] opacity-100 font-extrabold">
        <p class="text-justify hover:text-[#A284B6] fadeIn2s dark:text-slate-400 text-xl dark:text-white">Business <br> development</p>
    </div>
</div>

<div class="z-10 absolute mt-[-22%] left-56 transform -translate-x-1/2 -translate-y-1/2 text-center">
    <div class="shadow-md mb-4 p-10 bg-[#00000] opacity-100 font-extrabold">
        <p class="text-justify hover:text-[#A284B6] fadeIn2s dark:text-slate-400 text-xl dark:text-white" >Building tech</p>
    </div>
</div>

<div class="z-10 absolute mt-[-12%] left-72 transform -translate-x-1/2 -translate-y-1/2 text-center">
    <div class="shadow-md mb-4 p-10 bg-[#00000] opacity-100 font-extrabold">
        <p class="text-justify hover:text-[#A284B6] fadeIn2s dark:text-slate-400 text-xl dark:text-white">Hands on</p>
    </div>
</div>

<div class="z-10 absolute mt-[-50%] right-32 transform -translate-x-1/2 -translate-y-1/2 text-center">
    <div class="shadow-md mb-4 p-10 bg-[#00000] opacity-100 font-extrabold">
        <p class="text-justify hover:text-[#A284B6]  fadeIn2s  dark:text-slate-400 text-xl dark:text-white">Guiding clients <br> as partners </p>
    </div>
</div>

<div class="absolute mt-[-40%] right-2 transform -translate-x-1/2 -translate-y-1/2 text-center">
    <div class="shadow-md mb-4 p-10 bg-[#00000] opacity-100 font-extrabold">
        <p class="text-justify hover:text-[#A284B6] fadeIn2s dark:text-slate-400 text-xl dark:text-white">Tailoring counsel <br> to specific needs </p>
    </div>
</div>

<div class="z-10 absolute mt-[-22%] transform right-14 -translate-x-1/2 -translate-y-1/2 text-center">
    <div class="shadow-md mb-4 p-10 bg-[#00000] opacity-100 font-extrabold">
        <p class="text-justify hover:text-[#A284B6] fadeIn2s dark:text-slate-400 text-xl dark:text-white">Introducing <br> relationships</p>
    </div>
</div>

<div class="z-10 absolute mt-[-12%] right-20 transform -translate-x-1/2 -translate-y-1/2 text-center">
    <div class="shadow-md mb-4 p-10 bg-[#00000] opacity-100 font-extrabold">
        <p class="text-justify hover:text-[#A284B6] fadeIn2s dark:text-slate-400 text-xl dark:text-white">Raising capital</p>
    </div>
</div>
